...
// make entry js template configurable
function getEntryJsTemplate(config, key) {
  const item = config.entry[key];
  const configPath = item.entryJsTemplate || config.entryJsTemplate;
  const filepath = configPath ?
                    path.join(process.cwd(), configPath) :
                      path.join(__dirname, 'entry.nunjucks.js');
...
exports.start = function start(program) {
  ...
  Object.keys(config.entry).forEach((key) => {
    ...
    fs.writeFileSync(
      entryTemplatePath,
      nunjucks.renderString(getEntryJsTemplate(config, key), {
